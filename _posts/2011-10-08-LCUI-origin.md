...
头文件的编写，我先仿照了FreeType2的头文件编写风格，也就是先#include<XXX_Build.h>，要使用哪个头文件里的函数声明时，直接使用 #include _XX_XX_H_宏，这个宏但是XXX_Build.h里定义的；但遇到了一些问题，头文件包含头文件，循环包含，最终编译器编译时一直在循环打印同一个错误，关于这个问题，解决方法是使用宏进行头文件保护；
