...
用户可以手动导入问题（Issue），如果问题所属项目未被 GitDigger 收录，则创建一个。
...
注册用户可以对代码片段的复杂度评分，分数越高意味着代码越难被改进，满分 10 分，用 5 个级别难度标记：很容易、容易、普通、困难、很困难。代码片段的初始分数为 6。
